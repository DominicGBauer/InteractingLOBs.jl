...
L = 132     # real system width (e.g. 200 meters)
...
# # uncomment to rerun
# epps_2 = Empirical(epps_data)
# save("Computed Data/EppsCorrection/Empirical_delta_x_1_over_3.png.jld", "epps", epps_2)

# ComputedResults = load("Computed Data/EppsCorrection/Empirical_delta_x_1_over_3.png.jld")
# epps_2 = ComputedResults["epps"]
...
p2_x = plot(dt, average_epps_mean, legend = false,dpi=300, ribbon=(q .* std(average_epps_value, dims = 2) * 0.001), fillalpha=.15, title="Δt=0.03 Δx=1/3")

xlabel!(p2_x, L"\Delta t\textrm{[sec]}")
ylabel!(p2_x, L"\rho_{\Delta t}^{ij}")

savefig(p2_x, "Plots/Epps/Epps_delta_x_1_over_3.png")
