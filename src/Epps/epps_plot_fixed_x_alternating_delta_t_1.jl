...
# # uncomment to rerun
# epps_1_t = Empirical(epps_data)
# save("Computed Data/EppsCorrection/Empirical_delta_t_1.png.jld", "epps", epps_1_t)

# ComputedResults = load("Computed Data/EppsCorrection/Empirical_delta_t_1.png.jld")
# epps_1_t = ComputedResults["epps"]
...
p1_t = plot(dt, average_epps_mean, legend = false,dpi=300, ribbon=(q .* std(average_epps_value, dims = 2) * 0.001), fillalpha=.15, title="Δt=0.0625 Δx=1/2")
xlabel!(p1_t, L"\Delta t\textrm{[sec]}")
ylabel!(p1_t, L"\rho_{\Delta t}^{ij}")

savefig(p1_t, "Plots/Epps/Epps_delta_t_1.png")
