...
D = 0.8 # real diffusion constant e.g. D=1 (meters^2 / second), 1
...
# # uncomment to rerun
# epps_2_t = Empirical(epps_data)
# save("Computed Data/EppsCorrection/Empirical_delta_t_2.png.jld", "epps", epps_2_t)

# ComputedResults = load("Computed Data/EppsCorrection/Empirical_delta_t_2.png.jld")
# epps_2_t = ComputedResults["epps"]
...
p2_t = plot(dt, average_epps_mean, legend = false,dpi=300, ribbon=(q .* std(average_epps_value, dims = 2) * 0.001), fillalpha=.15, title="Δt=0.0781 Δx=1/2")
xlabel!(p2_t, L"\Delta t\textrm{[sec]}")
ylabel!(p2_t, L"\rho_{\Delta t}^{ij}")

savefig(p2_t, "Plots/Epps/Epps_delta_t_2.png")
