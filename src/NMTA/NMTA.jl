...
# ta_rounds = [5, 10, 20, 30, 35]
# objective = NonDifferentiable(x -> WeightedSumofSquaredErrors(Parameters(Nᴸₜ = Int(abs(ceil(x[1]))), Nᴸᵥ = Int(abs(ceil(x[2]))), δ = abs(x[3]), κ = abs(x[4]), ν = abs(x[5]), σᵥ = abs(x[6])), 5, W, empiricalmoments, empiricallogreturns, gateway), initialsolution)
# optimizationoptions = Options(show_trace = true, store_trace = true, trace_simplex = true, extended_trace = true, iterations = sum(ta_rounds), ξ = 0.15, ta_rounds = ta_rounds, f_reltol = f_reltol)
# optimizationoptions = nothing
# neldermeadstate = nothing
# @time result = !isnothing(neldermeadstate) ? Optimize(objective, initialsolution, nothing, neldermeadstate) : Optimize(objective, initialsolution, optimizationoptions)
...
#----- Process Point -----#
function ProcessPoint(simplexPoint::Vector{Float64})
    simplexPoint[1] = 400 # M
    simplexPoint[2] = 200 # L
    simplexPoint[3] = abs(simplexPoint[3]) # D
    simplexPoint[4] = abs(simplexPoint[4]) # ν
    simplexPoint[5] = max(min(abs(simplexPoint[5]), 1.0), 0.4) # γ
    return simplexPoint
end
#---------------------------------------------------------------------------------------------------
