...
mutable struct Parameters
    ...
    function Parameters(; M=400, L=200, D=0.5, ν=5.0, γ=1.0) # 1000, 25000
        new(M, L, D, ν, γ)
    end
end
...
function GenerateEmpericalReturnsAndMoments(startTime::DateTime, endTime::DateTime)
    ...
    filter!(x -> !ismissing(x.MidPrice), empericalData)
    filter!(x -> !ismissing(x.MicroPrice), empericalData)
...
function WeightedSumofSquaredErrors(parameters::Parameters, replications::Int64, W::Array{Float64,2}, empiricalmoments::Moments, empiricallogreturns::Vector{Float64})
    ...
    for i in 1:replications
        lob_model¹ = SLOB(1, s, p₀, parameters.M, parameters.L, parameters.D, parameters.ν, parameters.γ, mySourceTerm, myCouplingTerm, myRLPusher1, myRandomnessTerm)
        lob_model² = SLOB(1, s, p₀, parameters.M, parameters.L, parameters.D, parameters.ν, parameters.γ, mySourceTerm, myCouplingTerm, myRLPusher2, myRandomnessTerm)
...
function PlotObjectiveConvergence(stacktrace)
    ...
    f = zeros(Float64, iters)
    g_norm = zeros(Float64, iters)
    f_simplex = fill(0.0, iters, 6)#; centr = fill(0.0, length(stacktrace), 5); metadata = Vector{Dict{Any, Any}}()
    i = 1
    j = 1
