...
function dtrw_solver(slob¹::SLOB, slob²::SLOB, recalc)
    if (recalc)
        ...
        return dtrw_solver_no_recalc_fractional(slob¹, slob²)
    end
end
...
function intra_time_period_simulate_fractional(slob¹, φ_list¹, p_list¹, V_list¹, slob², φ_list², p_list², V_list²,
                                    t) 
    #p_listⁱ is a list of all prices until the most recent
    #φⁱ is the most recent order book only
    ...
    myend = size(φ_list¹[:,t-1],1) #end and myend are now the same in all the below
    middle = 2:(myend-1) #all indices excluding the first and last
    ...
    for m in max(1,t-slob¹.cut_off):(t-1)
        
        front = SibuyaKernelDP(t-m,slob¹)
        
        φ_next¹[1] += front * 
            (  P⁺¹ * φ_list¹[1,m] + P⁻¹ * φ_list¹[2,m] + P¹ * φ_list¹[1,m]  ) 
             

        φ_next¹[end] += front * 
            (  P⁻¹ * φ_list¹[end,m] + P⁺¹ * φ_list¹[end-1,m] + P¹ * φ_list¹[end,m]  )
             
        
        φ_next¹[middle] +=  front * 
            (  P⁺¹ * φ_list¹[middle.-1,m] + P⁻¹ * φ_list¹[middle.+1,m] + P¹ * φ_list¹[middle,m]  )   
    end
    
    φ_next¹ +=  exp( - slob¹.nu * slob¹.Δt) * φ_list¹[:,t-1] + slob¹.Δt * net_source¹
    ...
    for m in max(1,t-slob².cut_off):(t-1)
        #front = (fractional_choose(1-slob².γ , t-m ) * (-1)^(t-m) + (t-m==1 ? 1 : 0)) * exp(-(t-m)*slob².nu*slob².Δt)
        #front = SibuyaKernelModified(t-m,slob².γ,slob².nu,slob².Δt)
        front = SibuyaKernelDP(t-m,slob²)
        
        φ_next²[1] += front * 
            (P⁺² * φ_list²[1,m] + P⁻² * φ_list²[2,m] + (P²-1) * φ_list²[1,m]) 
             

        φ_next²[end] += front * 
            (P⁻² * φ_list²[end,m] + P⁺² * φ_list²[end-1,m] + (P²-1) * φ_list²[end,m] )
             
        
        φ_next²[middle] +=  front * 
            (P⁺² * φ_list²[middle.-1,m] + P⁻² * φ_list²[middle.+1,m] + (P²-1) * φ_list²[middle,m] )   
    end
...
function dtrw_solver_no_recalc_fractional(slob¹::SLOB, slob²::SLOB)
    broke_point = -1
    time_steps = to_simulation_time(slob¹.T, slob¹.Δt) 
    ...
    φ²[:, t], source²[:,t], coupling²[:,t], rl_push²[:,t], V²[t] = initial_conditions_numerical(slob², φ², p² ,
                                                                          ...
                                                                          t, 0.0) #get initial shape given at t=1
    t = 2
    while (t <= time_steps) && (!(p¹[t-1]==-1) || (p²[t-1]==-1)) #second condition occurs if it breaks
        φ¹[:, t], source¹[:,t], coupling¹[:,t], rl_push¹[:,t], P⁺s¹[t-1], P⁻s¹[t-1], Ps¹[t-1], V¹[t],  
        φ²[:, t], source²[:,t], coupling²[:,t], rl_push²[:,t], P⁺s²[t-1], P⁻s²[t-1], Ps²[t-1], V²[t] =
            intra_time_period_simulate_fractional(      slob¹, φ¹, p¹, V¹,
                                                        slob², φ², p², V²,
                                                        t                       )    #get φ at next time step
    ...
    return   φ¹, source¹, coupling¹, rl_push¹, p¹, sample_mid_prices¹, P⁺s¹, P⁻s¹, Ps¹, V¹,
             φ², source², coupling², rl_push², p², sample_mid_prices², P⁺s², P⁻s², Ps², V², broke_point
end
